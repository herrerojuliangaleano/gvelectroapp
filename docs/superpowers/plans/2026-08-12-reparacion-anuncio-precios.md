...
- [x] Crear un dump custom de la base `electrogv`.
- [x] Confirmar que existe lote `#93` con cuatro items.
- [x] Confirmar actualizaciones `984, 987, 988, 989, 990, 991`.
- [x] Confirmar productos `1380` y `1387` con SKU viejo/nuevo.
...
- [x] Reasociar `#984` a `products.id=1387` y corregir marca/SKU/descripcion.
- [x] Corregir descripciones de `#988` y `#989` desde `products`.
- [x] Cancelar `#990` y `#991` con motivo `identity_merge_duplicate`.
- [x] Desactivar `products.id=1380`.
- [x] Limpiar `announcement_archived_at/by` de `984,987,988,989`.
- [x] Corregir items del lote `#93` y `brand_names`.
- [x] Insertar eventos de historial y confirmar la transaccion.

### Task 3: Validacion operativa

- [x] Consultar `announcement_pending=true` conceptualmente mediante las mismas
  condiciones del endpoint y confirmar los cuatro registros.
- [x] Confirmar que `#984` conserva `$775.000 -> $800.000`.
- [x] Confirmar que `#990/#991` estan canceladas y no aparecen en la bandeja.
- [x] Regenerar el lote `#93` por API o ejecutar smoke del renderer.
- [x] Verificar que marcas del lote sean `ENOVA` y `TELEFUNKEN`.
