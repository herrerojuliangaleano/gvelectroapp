  ...
  return (
    <div className="mx-auto max-w-7xl">
      ...
      {canUpload && <form onSubmit={submitUpload} className="mb-5 rounded-3xl border border-slate-800 bg-slate-950/60 p-5">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-[1.2fr_150px_100px_140px_130px_1.4fr_auto]">
            ...
            <table className="min-w-full text-sm">
              ...
              <tbody className="divide-y divide-slate-800">
                {bulkPreview.items.map((item) => <tr key={item.file_name}>
                  <td className="px-4 py-3 font-semibold text-white"><div className="max-w-[180px] truncate">{item.file_name}</div></td>
                  ...
                  <td className="px-4 py-3"><input value={bulkDniOverrides[item.file_name] || ''} onChange={(e) => setBulkDniOverrides((prev) => ({ ...prev, [item.file_name]: e.target.value }))} placeholder="DNI manual" className="w-full rounded-xl border border-slate-700 bg-slate-900 px-3 py-2 text-xs sm:w-36" /></td>
                </tr>)}
              </tbody>
            </table>
          </div>
      ...
      <div className="overflow-hidden rounded-3xl border border-slate-800 bg-slate-950/60">
        <div className="overflow-x-auto">
          <table className="min-w-full text-sm">
            <thead className="bg-slate-900 text-xs uppercase text-slate-400"><tr><th className="px-4 py-3 text-left">Período</th><th className="px-4 py-3 text-left">Empleado</th><th className="hidden px-4 py-3 text-left sm:table-cell">Archivo</th><th className="px-4 py-3 text-left">Estado</th><th className="hidden px-4 py-3 text-left md:table-cell">Carga</th><th className="px-4 py-3 text-right">Acciones</th></tr></thead>
            <tbody className="divide-y divide-slate-800">
              {data.items.map((r) => <tr key={r.id} className="hover:bg-slate-900/50"><td className="px-4 py-3 font-bold text-white">{String(r.period_month).padStart(2, '0')}/{r.period_year}</td><td className="px-4 py-3"><div className="max-w-[140px] truncate font-semibold text-white sm:max-w-none">{r.employee_name || r.employee_username}</div><div className="text-xs text-slate-500">DNI {r.employee_dni || 'pendiente'}</div></td><td className="hidden px-4 py-3 text-slate-300 sm:table-cell"><div className="max-w-[160px] truncate">{r.file_name}</div></td><td className="px-4 py-3"><StatusBadge status={r.status} /></td><td className="hidden px-4 py-3 text-xs text-slate-400 md:table-cell">{r.uploaded_at ? new Date(r.uploaded_at).toLocaleString() : '-'}</td><td className="px-4 py-3 text-right"><div className="flex justify-end gap-2"><button onClick={() => openDetail(r)} className="rounded-lg border border-slate-700 px-3 py-2 text-xs font-bold hover:bg-slate-800">Detalle</button><button onClick={() => openFile(r)} className="rounded-lg bg-blue-500 px-3 py-2 text-xs font-bold text-white hover:bg-blue-400">Ver</button></div></td></tr>)}
